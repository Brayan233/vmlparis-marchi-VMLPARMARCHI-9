...
wp_enqueue_style( 'linotype_field_posts', str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname( __FILE__ ) ) . '/style.css', false, false, 'screen' );
wp_enqueue_script( 'linotype_field_posts', str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname( __FILE__ ) ) . '/script.js', array( 'jquery' ), false, true );
...
if ( $POST_LIST ) {
 	foreach( $POST_LIST as $POST_ITEM ) {
		...
		$language = '';
		if ( function_exists( 'pll_get_post_language' ) ) $language = ' [' . pll_get_post_language( $POST_ITEM['ID'], 'locale' ) . ']';
		
		array_push( $field['options']['data'], array( 'title' => $POST_ITEM['post_title'] . $language, 'value' => $POST_ITEM['ID'] ) );
...
<li class="reavis_library_selectize wp-field wp-field-selectize <?php if( $field['options']['fullwidthitem'] ) { echo 'fullwidthitem'; } ?> <?php if( $field['fullwidth'] ) { echo 'fullwidth'; } ?>" style="display: block;padding:<?php echo $field['padding']; ?>">

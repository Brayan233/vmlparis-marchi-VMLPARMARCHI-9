...
//load script
wp_enqueue_media();
wp_enqueue_style( 'field-editor', str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname( __FILE__ ) ) . '/style.css', false, false, 'screen' );
...
// _HANDYlOG($field['options']);
$field['options'] = array_merge(  $default_options, $field['options'] );

if ( ! class_exists( 'ADMINBLOCKS_editor' ) ) {

Class ADMINBLOCKS_editor {
...
$ADMINBLOCKS_editor = new ADMINBLOCKS_editor();

}

?>

<li class="linotype_field_editor wp-field wp-field-editor <?php if( $field['fullwidth'] ) { echo 'fullwidth'; } ?> <?php if( $field['padding'] == "0px" ) { echo 'zero-border'; } ?> <?php if( $field['options']['hide_tools'] ) { echo 'hide-tools'; } ?> <?php if( $field['options']['hide_toolbar'] ) { echo 'hide-toolbar'; } ?>" wp-field-id="<?php echo $field['id']; ?>" style="padding:<?php echo $field['padding']; ?>">
	...
	<div class="field-content" style="visibility:visible;">
		...
		<div class="field-input" >
			...
			$editor['textarea_name'] = $field['id'] . '-editor';
			...
			$editor['tinymce']['inline'] = false;
			$editor['tinymce']['forced_root_block'] = false;
			...
			//if ( $field['options']['style'] == 'gray' ) $editor['tinymce']['content_css'] = get_stylesheet_directory_uri() . '/editor-styles.css' . ',' . str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname( __FILE__ ) ) . '/style/gray.css';
	      ...
	      //   	});
				...
				ed.on("change", function(e) {
						...
						//if(ed.isDirty())
						ed.save();
						jQuery("#'.$field['id'].'").val( jQuery("#'.$field['id'].'-editor").val() ).change();
			...
			wp_editor( stripcslashes( $field['value'] ), $field['id'] . '-editor', $editor );

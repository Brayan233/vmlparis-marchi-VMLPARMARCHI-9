...
function GithubApiRequest($url, $post=FALSE, $headers=array()) {
  ...
  if(session('access_token'))
    $headers[] = 'Authorization: Bearer ' . get_option( "github_access_token", TRUE );
...
function github_oauth_callback()
{
	...
	if(get('code')) {
 	...
 	if(session('access_token')) 
 	{
		...
		update_option( "github_access_token", $_SESSION["access_token"]);
...
update_option( "github_key", 'Iv1.0fca348c0b380b4d' );
update_option( "github_secret", '4f6ca96a447d489c341691141ceb340dc21a814c' );
update_option( "github_app_name", 'LINOTYPE.IO' );


?>

<a style="margin-left: 200px;" href="<?php echo site_url() . '/wp-admin/admin-ajax.php?action=github_oauth_redirect'; ?>"><input type="button" value="Login Using Github" /> <?php echo get_option( "github_access_token", 'no token' ); ?></a>

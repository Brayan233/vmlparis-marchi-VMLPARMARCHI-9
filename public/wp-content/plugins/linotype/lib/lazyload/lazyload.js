...
(function() {
  ...
  // console.log('picture fallback loaded v4');
  
  if ( version && version <= 12) {
      console.log('no picture support for IE ' + version + ' : fallback exec : v1.5');
      ...
      imgs.forEach( function(element, index) {
          ...
          if ( fallback ) {
              image.src = fallback;
              image.className = image.className.replace("lazyload-fadein", "");
              var new_image = document.createElement('img');
              new_image.src = fallback;
              new_image.className = image.className.replace("lazyload-fadein", "");
              var new_wrapper = document.createElement('div');
              new_wrapper.id = element.id;
              new_wrapper.className = element.className;
              new_wrapper.appendChild( new_image );
              parent.replaceChild(new_wrapper, element);
              // console.log('replace src with ' + fallback );
          }
      });
  }

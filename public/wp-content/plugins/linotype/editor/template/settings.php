        ...
        <div class="linoadmin-col">
            ...
            <ul>
                <li class="linotype-field">
                    ...
                    <span class="customize-control-title">Remove supports</span>

                    <select name="supports[]" size="20" multiple="" style="width: 100%;">
                        
                        <?php 



                        foreach( array( 'title', 'editor', 'author', 'thumbnail', 'excerpt', 'trackbacks', 'custom-fields', 'comments', 'revisions', 'page-attributes', 'post-formats' ) as $support_key => $support ) {

                            $selected = '';
                            $LINOTYPE_EDITOR_data_supports = array();
                            if ( is_array( $LINOTYPE_EDITOR['data']['supports'] ) ) $LINOTYPE_EDITOR_data_supports = array_values( $LINOTYPE_EDITOR['data']['supports'] ); 
                            if ( in_array( $support, $LINOTYPE_EDITOR_data_supports ) ) $selected = 'selected';
                ...
                <li class="linotype-field">
                    ...
                    <select name="target[]" size="20" multiple="" style="width: 100%;">
                        
                        <?php 
                        foreach( LINOTYPE_helpers::get_theme_types() as $type_key => $type ) {

                            $selected = '';
                            $LINOTYPE_EDITOR_data_target = array();
                            if ( is_array( $LINOTYPE_EDITOR['data']['target'] ) ) $LINOTYPE_EDITOR_data_target = array_values( $LINOTYPE_EDITOR['data']['target'] ); 
                            if ( in_array( $type['value'], $LINOTYPE_EDITOR_data_target ) ) $selected = 'selected';

<?php
	...
	$hide_is_select = '';
	//if ( isset( $MAP[$map_key]['types'][$type_key]['rules'][$rule_key]['if'] ) && ! in_array( $MAP[$map_key]['types'][$type_key]['rules'][$rule_key]['if'], array( 'taxonomy', 'post' ) ) ) $hide_is_select = 'display:none;';
		...
		if ( isset( $map['infos']['taxonomies'] ) && $map['infos']['taxonomies'] ){
			foreach ( $map['infos']['taxonomies'] as $taxonomie_key => $taxonomie ) {
				...
				if ( $taxonomie ){
					...
					$hide_option = '';
					//if ( $MAP[$map_key]['types'][$type_key]['rules'][$rule_key]['if'] !== 'taxonomy' ) $hide_option = 'display:none;';
	...
	$hide_is_text = 'display:none;';
	//if ( in_array( $MAP[$map_key]['types'][$type_key]['rules'][$rule_key]['if'], array( 'taxonomy', 'post' ) ) ) $hide_is_text = 'display:none;';

	echo '<input type="text" style="' . $hide_is_text . '" class="template-field rule-is" value="" style="min-width:20%">';

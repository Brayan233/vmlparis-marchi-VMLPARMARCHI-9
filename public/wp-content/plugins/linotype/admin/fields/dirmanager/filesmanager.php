...
$root_path = $dirmanager_options['dir'];

$root_dirname = dirname( $root_path );
...
$root_url = str_replace( array( 'https://' . $http_host, 'http://' . $http_host ), '', $dirmanager_options['url'] );
...
defined('FM_SELF_URL') || define('FM_SELF_URL', ($is_https ? 'https' : 'http') . '://' . $http_host . $_SERVER['PHP_SELF'] . '?dirmanager_id=' . $_REQUEST['dirmanager_id'] );
    ...
    <div class="path">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <p>
                ...
                <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> <?php echo fm_t('Cancel') ?></a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <p>
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> <?php echo fm_t('Copy') ?></a></b> &nbsp;
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> <?php echo fm_t('Move') ?></a></b> &nbsp;
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> <?php echo fm_t('Cancel') ?></a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            if ($parent !== false) {
                ?>
                <li><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode($parent) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></li>
            <?php
            }
            foreach ($folders as $f) {
                ?>
                <li><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php echo fm_convert_win($f) ?></a></li>
            <?php
            }
            ?>
        </ul>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p>
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="fa fa-cloud-download"></i> <?php echo fm_t('Download') ?></a></b> &nbsp;
            ...
            if (!FM_READONLY && $is_zip && $filenames !== false) {
                ...
                ?>
                <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> <?php echo fm_t('UnZip') ?></a></b> &nbsp;
                <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="<?php echo fm_t('UnZip to') ?> <?php echo fm_enc($zip_name) ?>"><i class="fa fa-check-circle"></i>
                    <?php echo fm_t('UnZip to folder') ?></a></b> &nbsp;
                <?php
            }
            if($is_text && !FM_READONLY) {
            ?>
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo fm_t('Edit') ?></a></b> &nbsp;
            <?php }
            ...
            <?php } ?>
            <b><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-chevron-circle-left"></i> <?php echo fm_t('Back') ?></a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
...
if (isset($_GET['edit'])) {
    ...
    <div class="path">
        <div class="edit-file-actions">
            <a title="Cancel" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> Cancel</a>
            ...
            <?php if($is_text) { ?>
                <?php if($isNormalEditor) { ?>
                    <a title="Advanced" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-paper-plane"></i> Advanced Editor</a>
                    <button type="button" name="Save" data-url="<?php echo $file_url ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Save</button>
                <?php } else { ?>
                    <a title="Plain Editor" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> Plain Editor</a>
...
if ($parent !== false) {
    ?>
<tr><?php if (!FM_READONLY): ?><td><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode($parent) ?>"><i class="fa fa-chevron-circle-left"></i></a></td><?php endif; ?><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode($parent) ?>">..</a></td></tr>
<?php
}

foreach ($folders as $f) {
    $is_link = is_link($path . '/' . $f);
    $img = $is_link ? 'icon-link_folder' : 'fa fa-folder icon-folder icon-folder';
...
<td><div class="filename"><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <b><?php echo fm_convert_win($f) ?></b></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div></td>
<td><?php echo fm_t('Folder') ?></td><td><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
<td><?php if (!FM_READONLY): ?><a title="<?php echo fm_t('Change Permissions') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?></td>
<td><?php echo $owner['name'] . ':' . $group['name'] ?></td>
<?php endif; ?>
<td class="inline-actions"><?php if (!FM_READONLY): ?>
<a title="<?php echo fm_t('Delete') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('<?php echo fm_t('Delete folder?') ?>');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
...
<a title="<?php echo fm_t('Copy to...') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
<?php endif; ?>
...
foreach ($files as $f) {
    ...
    $filelink = '?dirmanager_id=' . $_REQUEST['dirmanager_id'] . '&p=' . urlencode(FM_PATH) . '&amp;view=' . urlencode($f);
...
<?php if (!FM_READONLY): ?>
<a title="<?php echo fm_t('Delete') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('<?php echo fm_t('Delete file?') ?>');"><i class="fa fa-trash-o"></i></a>
...
<a title="<?php echo fm_t('Copy to...') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
<?php endif; ?>
<a title="<?php echo fm_t('Direct link') ?>" href="<?php echo FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f ?>" target="_blank"><i class="fa fa-link"></i></a>
<a title="<?php echo fm_t('Download') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
<?php
    ...
    ?>
<tr><?php if (!FM_READONLY): ?><td class=""></td><?php endif; ?><td class="" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
...
<div class="float-right2">
        ...
        $pathroot = 'root';
        $path = fm_clean_path($path);
        $sep = '<span style="color:#999;">&nbsp;&nbsp;/&nbsp;&nbsp;</span>';
        $root_url = "<a href='?dirmanager_id=" . $_REQUEST['dirmanager_id'] . "&p='>" . $pathroot . $sep . "</a>";
        if ($path != '') {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $array[] = "<a href='?dirmanager_id=" . $_REQUEST['dirmanager_id'] . "&p={$parent_enc}'>" . fm_convert_win($exploded[$i]) . "</a>";
            }
            $root_url .= implode($sep, $array);
        }
...
<?php if (!FM_READONLY): ?>
<div class="path footer-links"><!-- <a href="#/select-all" class="group-btn" onclick="select_all();return false;"><i class="fa fa-check-square"></i> <?php echo fm_t('Select all') ?></a> &nbsp;
...
<a href="javascript:document.getElementById('a-delete').click();" class="group-btn"><i class="fa fa-trash"></i></a>
...
<a href="javascript:document.getElementById('a-zip').click();" class="group-btn"><i class="fa fa-file-archive-o"></i> </a>
<input type="submit" class="hidden" name="copy" id="a-copy" value="<?php echo fm_t('Copy') ?>">
<a href="javascript:document.getElementById('a-copy').click();" class="group-btn"><i class="fa fa-files-o"></i></a>
<!-- <a href="https://github.com/prasathmani/tinyfilemanager" target="_blank" class="float-right">H3K | Tiny File Manager</a>-->
<span style="color:#999;">&nbsp;&nbsp;|&nbsp;&nbsp;</span>
<?php if (!FM_READONLY): ?>
<a title="<?php echo fm_t('Search') ?>" href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-search"></i></a>
<a title="<?php echo fm_t('Upload files') ?>" href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i></a>
...
html{overflow:hidden}body{padding:0;font:13px/16px Tahoma,Arial,sans-serif;color:#222;background:#transparent}
...
th,td{padding:4px 7px;text-align:left;vertical-align:top;border:1px solid #ddd;background:transparent;white-space:nowrap}
th,td.gray{background-color:#eee}td.gray span{color:#222}
tr:hover td{background-color:transparent}tr:hover td.gray{background-color:#eee}
.table{width:100%;max-width:100%;margin-bottom:1rem}.table td,.table th{padding:.55rem;vertical-align:top;border-top:1px solid #ddd}.table thead th{vertical-align:bottom;border-bottom:2px solid #eceeef}.table tbody+tbody{border-top:2px solid #eceeef}.table .table{background-color:transparent}
...
body {margin:0px;margin-top: 36px;}
#wrapper{margin:0 auto}
.path{padding:4px 7px;border:1px solid #ddd;background-color:transparent;margin-bottom:10px}
.right{text-align:right}.center{text-align:center}.float-right{float:right}.float-left{float:left}
.message{padding:4px 7px;border:1px solid #ddd;background-color:transparent}
...
.compact-table{border:0;width:auto}.compact-table td,.compact-table th{width:100px;border:0;text-align:center}.compact-table tr:hover td{background-color:transparent}
...
.group-btn{background:transparent;padding:2px 6px;border:1px solid;cursor:pointer;color:#296ea3;}
...
http.send(params);}
var searchEl = document.querySelector('input[type=search]');var timeout = null;searchEl.onkeyup = function(evt) {clearTimeout(timeout);var data = JSON.parse(window.searchObj);var searchTerms = document.querySelector('input[type=search]').value;timeout = setTimeout(function () {if(searchTerms.length>=2) {var res = getSearchResult(data,searchTerms);var f1='',f2='';res.folders.forEach(function (d){f1+='<li class="'+d.type+'"><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p='+ d.path+'">'+d.name+'</a></li>'; });res.files.forEach(function (d){f2+='<li class="'+d.type+'"><a href="?dirmanager_id=<?php echo $_REQUEST['dirmanager_id']; ?>&p='+ d.path +'&view='+ d.name +'">'+d.name+'</a></li>';});document.getElementById('searchresultWrapper').innerHTML = '<div class="model-wrapper">'+f1+f2+'</div>';}},500);};

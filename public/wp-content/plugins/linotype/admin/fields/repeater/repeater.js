...
$.fn.wp_field_repeater = function(){
	...
	$(this).each(function(){
		...
		var $NESTABLE_TREE = $FIELD.find('.repeater-tree');
		var $NESTABLE_VALUE = $FIELD.find('.repeater-value');
		var $NESTABLE_ITEMS = $FIELD.find('.item-settings');
		...
		$FIELD.on( 'click', '.repeater-source-view', showSource ) ;
		...
		*/
		$NESTABLE_TREE.nestable({
			...
			'expandBtnHTML':'<div class="dd-action dashicons dashicons-arrow-right" data-action="expand"></div>',
			'collapseBtnHTML':'<div class="dd-action dashicons dashicons-arrow-down" data-action="collapse"></div>',
		});
		...
		*/
		$NESTABLE_TREE.on('click', '.dd-remove', function(){
			...
			$NESTABLE_TREE.nestable( 'refresh' );
		...
		*/
		$NESTABLE_TREE.on('click', '.dd-edit', function(){
			...
			if ( ! $content.find('.item-settings').length ) {
			
				$data = getDataAttributes($item);

				$NESTABLE_ITEM_SETTINGS = '<div class="item-settings" style="display:none;">';
						...
						$.each( $data, function( index, attr ) {
							...
							if ( index !== 'id' && index !== 'children' ) {
								...
								if ( index == 'icon' ) {
									...
									$NESTABLE_ITEM_SETTINGS += '<div class=""><label>' + index + '</label><br/>';
									...
									$NESTABLE_ITEM_SETTINGS += '<div class=""><label>' + index + '</label><br/><input class="dd-option widefat" data-target="data-' + index + '" type="text" value="' + attr + '" autocomplete="off"></div>';
				...
				$content.find('.item-settings-holder').append( $NESTABLE_ITEM_SETTINGS );

			}

			$content.find('.item-settings').slideToggle(300);

			$content.find('.item-settings').find('.icon-picker').icon_picker();
		...
		*/
		...
		$('#repeater-lib li').on('click', function(){
		
			//$('#repeater-enable > .dd-list').append( $(this)[0].outerHTML );
